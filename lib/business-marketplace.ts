...
function dateValue(row: Row, key: string, optional = false) {
  ...
  if (!Number.isFinite(new Date(value).getTime())) throw new Error(`Invalid ${key}`);
  return value;
}
...
export function mapMarketplaceControls(value: unknown): MarketplaceControls {
  ...
  if (kind !== 'home_kitchen' && kind !== 'pop_up') throw new Error('Invalid business kind');
  if (typeof row.chat_enabled !== 'boolean' || typeof row.chat_required !== 'boolean' || typeof row.can_toggle_chat !== 'boolean') {
...
export function mapManagedPickupSites(value: unknown): ManagedPickupSite[] {
  ...
  return value.map((candidate) => {
    ...
    if (kind !== 'public_meeting_place' && kind !== 'commercial_site') throw new Error('Invalid pickup site kind');
    if (state !== 'submitted' && state !== 'approved' && state !== 'rejected') throw new Error('Invalid pickup site state');
    return {
...
export function validatePickupSiteDraft(draft: PickupSiteDraft) {
  const clean = {
    ...
    kind: draft.kind,
    addressLine: draft.addressLine.normalize('NFKC').replace(/\s+/g, ' ').trim(),
  ...
  if (!clean.label || clean.label.length > 120) throw new Error('Use a short public place label, up to 120 characters.');
  if (!clean.addressLine || clean.addressLine.length > 300) throw new Error('Enter the exact street address of the public or commercial meeting place.');
  if (!clean.city || clean.city.length > 120 || !clean.region || clean.region.length > 80) throw new Error('Enter a valid city and state, province, or region.');
  if (clean.postalCode.length > 24) throw new Error('The postal code is too long.');
  if (!Number.isFinite(clean.latitude) || clean.latitude < -90 || clean.latitude > 90 || !Number.isFinite(clean.longitude) || clean.longitude < -180 || clean.longitude > 180) {
...
export function createMarketplaceOperationsKey(action: string) {
  ...
  const nonce = cryptoApi?.randomUUID?.() ?? `${Date.now().toString(36)}-${keySequence.toString(36)}-${Math.random().toString(36).slice(2)}`;
...
function failure<T>(error: unknown, fallback: string): BusinessMarketplaceResult<T> {
  const candidate = error as ErrorLike;
  const message = candidate?.message?.toLocaleLowerCase('en-US') ?? '';
  if (candidate?.status === 401 || message.includes('jwt') || message.includes('not authenticated')) return { ok: false, code: 'AUTH', reason: 'Sign in again to manage marketplace safety.' };
  if (candidate?.status === 403 || candidate?.code === '42501' || message.includes('aal2') || message.includes('manager')) return { ok: false, code: 'FORBIDDEN', reason: message.includes('aal2') ? 'Verify a current authenticator code before managing private pickup details.' : 'Only an owner or manager can change these controls.' };
  if (candidate?.code === '40001' || message.includes('changed')) return { ok: false, code: 'CONFLICT', reason: 'This pickup site changed. Reload before trying again.' };
  if (candidate?.code === '22023' || candidate?.code === '23514' || message.includes('invalid') || message.includes('sensitive')) return { ok: false, code: 'INVALID', reason: fallback };
  if (message.includes('fetch') || message.includes('network')) return { ok: false, code: 'NETWORK', reason: 'Spottr could not be reached. Check the connection and retry.' };
...
export async function setBusinessMarketplaceChat(businessId: string, enabled: boolean): Promise<BusinessMarketplaceResult<boolean>> {
  try {
    const client = await secureClient();
    const { data, error } = await client.rpc('set_business_marketplace_chat_enabled', { target_business_id: businessId, should_enable: enabled, idempotency_key: createMarketplaceOperationsKey('chat') });
    if (error) throw error;
    const row = rowFrom(data);
    if (typeof row.enabled !== 'boolean') throw new Error('Invalid chat setting receipt');
...
export async function submitPickupSite(businessId: string, draft: PickupSiteDraft): Promise<BusinessMarketplaceResult<string>> {
  try {
    const clean = validatePickupSiteDraft(draft);
    const client = await secureClient();
    const { data, error } = await client.rpc('submit_marketplace_pickup_site', {
      target_business_id: businessId, site_label: clean.label, site_kind: clean.kind,
      address_line: clean.addressLine, city: clean.city, region: clean.region,
      postal_code: clean.postalCode || null, latitude: clean.latitude, longitude: clean.longitude,
      idempotency_key: createMarketplaceOperationsKey('site'),
    });
    if (error) throw error;
    return { ok: true, data: uuidValue(rowFrom(data), 'pickup_site_public_id') };
  } catch (error) {
    return failure(error, error instanceof Error ? error.message : 'The pickup site could not be submitted.');
  }
}

export async function archivePickupSite(site: ManagedPickupSite): Promise<BusinessMarketplaceResult<void>> {
  try {
    const client = await secureClient();
