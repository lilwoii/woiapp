...
const APPROVED_MAP_CSP_TEST_FILE = 'lib/__tests__/map-csp.test.ts';
const APPROVED_HOSTING_WORKER_TEST_FILE = 'scripts/hosting-worker.test.mjs';
const APPROVED_FILES = new Set([
  APPROVED_CONFIG_FILE,
  APPROVED_LINK_TEST_FILE,
  APPROVED_MAP_CSP_TEST_FILE,
  APPROVED_HOSTING_WORKER_TEST_FILE,
]);
const APPROVED_FINDING_GROUPS = new Map([
  ['f6ed53c894e8cf577d965314ffc8d3b2a115fe80:scripts/verify-production-config-gate.mjs:87:PLAIN:a7f6887e112e6db66533958fb2ebe693c12ac696c52839a917103046ea3d5409', 'config-f6ed53c'],
  ['f6ed53c894e8cf577d965314ffc8d3b2a115fe80:scripts/verify-production-config-gate.mjs:90:PLAIN:c40d05f65009500bb1583fba187eb601f44faa2ba20a948c971f7ba1869478fa', 'config-f6ed53c'],
  ['59ed1ca33d55dcd3c8df19adb3cc09fc9ca58d63:scripts/verify-production-config-gate.mjs:87:PLAIN:a7f6887e112e6db66533958fb2ebe693c12ac696c52839a917103046ea3d5409', 'config-59ed1ca'],
  ['59ed1ca33d55dcd3c8df19adb3cc09fc9ca58d63:scripts/verify-production-config-gate.mjs:90:PLAIN:c40d05f65009500bb1583fba187eb601f44faa2ba20a948c971f7ba1869478fa', 'config-59ed1ca'],
  ['ef8c8780ea4d447b4a91a5cf0ec94cf6672db6e8:lib/__tests__/links.test.ts:7:PLAIN:8d3331ee208c72c30fba199e4e2b8a65d69a5034e49875a2f20dbea3a4f2f976', 'links-ef8c878'],
  ['94ff4544bfe3c8ee3df911a0e4160c1a12c0b64f:lib/__tests__/map-csp.test.ts:19:PLAIN:7523ef6db4fabf8b89d010a4f687c4503f1878bc534916335b9421ae2346c544', 'map-csp-94ff454'],
  ['94ff4544bfe3c8ee3df911a0e4160c1a12c0b64f:scripts/hosting-worker.test.mjs:64:HTML:7523ef6db4fabf8b89d010a4f687c4503f1878bc534916335b9421ae2346c544', 'hosting-map-csp-94ff454'],
  ['94ff4544bfe3c8ee3df911a0e4160c1a12c0b64f:scripts/hosting-worker.test.mjs:67:PLAIN:7523ef6db4fabf8b89d010a4f687c4503f1878bc534916335b9421ae2346c544', 'hosting-map-csp-94ff454'],
  ['94ff4544bfe3c8ee3df911a0e4160c1a12c0b64f:scripts/verify-production-config-gate.mjs:106:PLAIN:029e5974b38d7fc6b62951393f61657725960f18237071e5a8fa922c5600760e', 'config-map-csp-94ff454'],
]);
...
export function validateTruffleHogOutput({
  ...
  for (const line of lines) {
    ...
    if (!isRecord(finding) || !isRecord(git)
      ...
      || !['PLAIN', 'HTML'].includes(finding.DecoderName)
    ...
    const key = `${git.commit}:${git.file}:${git.line}:${finding.DecoderName}:${sha256(finding.RawV2)}`;
