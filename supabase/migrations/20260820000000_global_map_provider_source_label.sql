-- Spottr global viewport licensed-provider source-label repair.
-- Replaces the public RPC without rewriting an already-applied migration.
...
do $provider_source_label_contract$
declare
...
begin
  ...
  if position('licensed_provider' in function_definition) = 0 then
    raise exception 'map_food_places must recognize licensed_provider provenance';
  end if;
end;
$provider_source_label_contract$;
