-- Forward-only repair for geography viewport filtering.
--
-- The generic bounding-box operator is not defined for geography in the
-- pinned Supabase/PostGIS runtime. ST_Intersects supports geography, includes
-- an index-aware bounding-box comparison, and the exact latitude/longitude
-- predicates below retain the intended viewport semantics.
