-- Mobile vendors may retain several approved reusable stop pins, but the public
-- map must show only where they are currently serving. Match nearby discovery:
-- prefer the deterministic active scheduled/live stop, otherwise use the
-- eligible primary fallback. Fixed-location businesses retain all locations.
