...
Deno.test("provider ingest RPC is a service-role-only transactional boundary", async () => {
  ...
  assert(
    source.includes("create function public.ingest_licensed_provider_batch("),
  );
  ...
  assert(
    /from public, anon, authenticated;\s+grant execute on function public\.ingest_licensed_provider_batch/
      .test(source),
  );
...
Deno.test("provider snapshot completion is ordered and non-destructive", async () => {
  ...
  assert(
    source.includes("snapshot_session.next_page_index <> sync_page_index"),
  );
...
Deno.test("provider materialization keeps owner precedence and drafts private", async () => {
  ...
  assert(
    source.includes("current_field_hash <> field_row.materialized_value_hash"),
  );
...
Deno.test("provider receipt exposes only the Edge-safe response shape", async () => {
  const source = await Deno.readTextFile(migrationUrl);
  const safeResponseStart = source.indexOf(
...
Deno.test("provider signing-key constraints use an immutable helper", async () => {
  ...
  const constraintStart = source.indexOf(
    "constraint provider_accounts_signing_keys_valid",
  );
  const relativeConstraintEnd = source.slice(constraintStart).search(
    /\r?\n  \)\r?\n\);/,
  );
  const constraintEnd = relativeConstraintEnd < 0
    ? -1
    : constraintStart + relativeConstraintEnd;
  assert(constraintStart >= 0 && constraintEnd > constraintStart);
  const constraint = source.slice(constraintStart, constraintEnd);

  assert(
    source.includes(
      "create or replace function private.provider_signing_key_ids_valid",
    ),
  );
  assert(
    constraint.includes(
